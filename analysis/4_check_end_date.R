...
read <- function(data){
  read_dta(here::here("output","cohorts",data)) %>%
    mutate(
        ...
        index_date = as.Date(sick_note_end_date, format = "%Y-%m-%d"),
        end_month  = floor_date(as.Date(sick_note_end_date), unit="month"),
        index_month =  floor_date(as.Date(index_date), unit="month")
          ) %>% 
...
write_csv(covid20_tab, here::here("output", "released_outputs", "end_date_covid20.csv"))

covid21_tab <- tab(covid21)
write_csv(covid21_tab, here::here("output", "released_outputs", "end_date_covid21.csv"))

gen19_tab <- tab(gen19)
write_csv(gen19_tab, here::here("output", "released_outputs", "end_date_gen19.csv"))

gen20_tab <- tab(gen20)
write_csv(gen20_tab, here::here("output", "released_outputs", "end_date_gen20.csv"))

gen21_tab <- tab(gen21)
write_csv(gen21_tab, here::here("output", "released_outputs", "end_date_gen21.csv"))
